...
void CACHE::upper_invalidate(BLOCK** block, uint32_t set, uint32_t way, uint32_t fill_cpu, uint32_t mshr_index){
  if(block[set][way].valid!=0){
    uint64_t addr = block[set][way].address;
    // cout<<cache_type<<endl;
    if(cache_type==IS_LLC){
      for(int i=0; i<NUM_CPUS; i++){
        MEMORY *l2 = upper_level_icache[i];
        PACKET *packet = new PACKET();
        packet->cpu = i;
        packet->address = block[set][way].address;
        packet->full_addr = block[set][way].full_addr;
        
        // if(l2->check_hit(packet)>(-1)){
          MEMORY *l1i = l2->upper_level_icache[i];
          MEMORY *l1d = l2->upper_level_dcache[i];
          if(l1i->check_hit(packet)>(-1)){
            if((l1i->is_dirty(packet)>0)){
            ...
            // l1i->invalidate_entry(addr);
          }
          else if(l1d->check_hit(packet)>(-1)){
            if((l1d->is_dirty(packet)>0)){
              PACKET writeback_packet;
            ...
            // l1d->invalidate_entry(addr);
          }
          else if(l2->check_hit(packet)>(-1)){
            if((l2->is_dirty(packet)>0)){
              PACKET writeback_packet;
          ...
          l1d->make_undirty(packet);
          l1i->make_undirty(packet);
          l2->make_undirty(packet);
          make_undirty(packet);
          l2->invalidate_entry(addr);
          l1i->invalidate_entry(addr);
          l1d->invalidate_entry(addr);
        }
      }
      ...
      packet->address = block[set][way].address;
      packet->full_addr = block[set][way].full_addr;
      MEMORY *l1i = upper_level_icache[fill_cpu];
      MEMORY *l1d = upper_level_dcache[fill_cpu];
      if(l1i->check_hit(packet)>(-1)){
        if((l1i->is_dirty(packet)>0)){
            PACKET writeback_packet;
...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        upper_invalidate(block, set, way, fill_cpu, mshr_index);
// #ifdef LLC_BYPASS
...
void CACHE::operate()
{
  ...
  // else if(cache_type==IS_L2C){
  //   handle_fill();
  //   MEMORY *l3 = lower_level;
  //   l3->handle_fill();
  // }
  // else if(cache_type==IS_L1D){
  //   handle_fill();
  //   MEMORY *l2 = lower_level;
  //   l2->handle_fill();
  //   MEMORY *l3 = l2->lower_level;
  //   l3->handle_fill();
  // }
  // else if(cache_type==IS_L1I){
  //   handle_fill();
...
int CACHE::is_dirty(PACKET *packet){
...
void CACHE::make_undirty(PACKET *packet){
    uint32_t set = get_set(packet->address);
    int way = check_hit(packet);
    if(way>=0)
      block[set][way].dirty = 0;
