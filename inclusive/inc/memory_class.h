...
class MEMORY {
  public:
    ...
    virtual int  check_hit(PACKET *packet) = 0;
    virtual int invalidate_entry(uint64_t inval_addr) = 0;
    virtual void handle_fill() = 0;
    virtual int is_dirty(PACKET *packet) = 0;
    virtual void make_undirty(PACKET *packet) = 0;
